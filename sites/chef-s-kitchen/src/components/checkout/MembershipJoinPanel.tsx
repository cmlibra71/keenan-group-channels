...
export function MembershipJoinPanel({
      ...
      <div className="mt-6 rounded-lg border border-brand-light/40 bg-brand-tint p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-brand-deep">{memberLine}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`mt-6 rounded-lg border p-5 ${free ? "border-brand-light/40 bg-brand-tint" : "border-member/40 bg-member-bg"}`}>
      <div className="flex items-start gap-3">
        <Crown className={`mt-0.5 h-5 w-5 shrink-0 ${free ? "text-brand" : "text-member-text"}`} />
        <div className="min-w-0">
          <p className={`font-semibold ${free ? "text-brand-deep" : "text-member-text"}`}>{join.headline}</p>
          {join.detail && (
            <p className={`mt-1 text-sm ${free ? "text-brand-deep" : "text-member-text"}`}>{join.detail}</p>
          )}
          ...
          {!join.namesPrice && planPriceLine && (
            <p className="mt-1 text-sm text-member-text">
          ...
          <p className="mt-1 text-sm">
            <Link
              href="/membership"
              className={`font-medium underline hover:no-underline ${free ? "text-brand-deep" : "text-member-text"}`}
            >
              What&apos;s included
            </Link>
          </p>
      ...
      <label
        className={`mt-4 flex items-start gap-3 rounded-lg border bg-white p-3 ${
          free ? "border-brand-light/40" : "border-member/40"
        }`}
      >
        <input
          type="checkbox"
        ...
        <span className="text-sm font-medium text-ink-900">{join.cta}</span>
      </label>
      ...
      {joining && (
        <div className="mt-4 space-y-4 rounded-lg bg-white p-4">
          ...
          <p className="rounded-lg bg-steel-50 px-3 py-2 text-xs text-text-secondary">{MEMBERSHIP_JOIN_NOTHING_CHARGED}</p>
        </div>
      )}
      ...
      {!isSignedIn && (
        <p className={`mt-3 text-xs ${free ? "text-brand-deep" : "text-member-text"}`}>
          Already a member?{" "}
          <Link
