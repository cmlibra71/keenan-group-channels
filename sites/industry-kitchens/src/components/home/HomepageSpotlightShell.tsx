"use client";
// The spotlight's CHROME — hero panel or heading row — with the product grid
// passed in as a slot.
//
// It exists because the two render paths need different grids: the live page
// uses the server ProductGrid (which applies catalog scope and account prices
// at read time), while a Site Builder native renders client-side and must use
// ProductGridClient with those passes already done upstream. Splitting the
// chrome out means ONE implementation of the markup instead of a copy that
// drifts — which matters, because the whole promise here is that the node
// homepage is pixel-identical to the live one.
...
export function HomepageSpotlightShell({
  heading,
  subheading,
  ctaHref,
  ctaLabel = "View all",
  ...
  ctaLabel?: string;
  hero?: HomeImage;
  /** The product grid — server or client, whichever the caller can use. */
