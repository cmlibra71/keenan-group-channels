...
// ============================================================================
// The homepage's inline section pieces, lifted out of HomeSections.tsx.
//
// They were plain functions inside that file, which made them unreachable from
// anywhere else — including the Site Builder natives that have to render the
// SAME markup, or the node homepage would not be pixel-identical to the live
// one. Extracted rather than copied: a second copy would drift, and the whole
// point is that both paths render one implementation.
//
// "use client" because a builder native renders inside a client tree. These are
// pure presentational functions — no data fetching, no server APIs — so the
// boundary move changes what ships to the browser, not what is drawn.
// ============================================================================
...
export function LogoStrip({ heading, logos }: { heading?: string; logos: HomeImage[] }) {
...
export function Testimonials({ heading, images }: { heading?: string; images: HomeImage[] }) {
