...
// Faithful reproduction of the template's legacy /pages/[slug] article — keep in
...
const ValueBarAdapter: FC<BlockProps> = ({ props }) => {
  const items = Array.isArray(props.items)
    ? (props.items as Array<{ icon?: string; label?: string }>)
    : [];
  return <ValueBar items={items.map((it) => ({ icon: str(it.icon), label: str(it.label) }))} />;
};
