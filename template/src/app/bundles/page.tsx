...
export default async function BundlesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {bundles.length === 0 ? (
        <p className="mt-8 text-zinc-500">There are no bundles running at the moment.</p>
      ) : (
        <ul className="mt-8 space-y-4">
          {bundles.map((bundle) => (
            <li key={bundle.slug} className="rounded-lg border border-zinc-200 p-6">
              <Link href={`/bundles/${bundle.slug}`} className="text-lg font-semibold text-zinc-900 hover:underline">
                {bundle.headline}
              </Link>
              <p className="mt-1 text-sm font-medium text-green-700">{bundle.percent}% off the bundle</p>
