...
export function ActivateMembershipForm({
  ...
  const primary =
    "w-full rounded-lg bg-zinc-900 px-4 py-3 font-semibold text-white hover:bg-zinc-800 disabled:opacity-50";
    ...
    <form onSubmit={submit} className="space-y-6">
      ...
      <section className="border-b border-zinc-200 pb-6">
        <div className="flex items-center justify-between">
          <h2
            className={`text-lg font-semibold ${
              step === "account" ? "text-zinc-400" : "text-zinc-900"
            }`}
          >
        ...
        {step === "membership" && addressLine && (
          <p className="mt-2 text-sm text-zinc-600">{addressLine}</p>
        )}
      ...
      <section>
        <h2
          className={`text-lg font-semibold ${
            step === "membership" ? "text-zinc-900" : "text-zinc-400"
          }`}
        >
