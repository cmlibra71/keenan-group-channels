  ...
  return (
    <div className="rounded-lg border border-steel-200 p-4">
      <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-ink-900">
        <Tag className="h-4 w-4 text-steel-400" /> Offers
      </h2>

      {applied.map((m) => (
        <p key={m.text} className="mb-2 text-sm font-medium text-brand">
          {m.text}
        </p>
      ))}
      {progress.map((m) => (
        <p key={m.text} className="mb-2 text-sm text-steel-600">
          {m.text}
        </p>
      ))}
      {quoteAsks.map((m) => (
        ...
        <ul className="mb-3 space-y-1">
          {couponCodes.map((applied) => (
            <li key={applied} className="flex items-center justify-between text-sm">
              <span className="font-medium text-brand">{applied}</span>
              <button
                type="button"
                onClick={() => drop(applied)}
                disabled={isPending}
                aria-label={`Remove promotion code ${applied}`}
                className="text-steel-400 hover:text-red-600 disabled:opacity-50"
              >
                <X className="h-4 w-4" />
              </button>
            </li>
      ...
      <form onSubmit={submit} className="flex gap-2">
        ...
        <input
          ...
          autoComplete="off"
          className="min-w-0 flex-1 rounded border border-steel-300 px-3 py-2 text-sm uppercase placeholder:normal-case focus:border-steel-500 focus:outline-none"
        />
        <button
          type="submit"
          disabled={isPending || code.trim() === ""}
          className="rounded bg-ink-900 px-4 py-2 text-sm font-semibold text-white hover:bg-ink-800 disabled:opacity-50"
        >
          Apply
        </button>
      </form>
