...
const inputClass =
  "w-full px-4 py-2 border border-steel-200 rounded-lg focus:ring-2 focus:ring-brand focus:border-brand text-sm";
...
export function FinanceApplicationPanel({
  ...
  return (
    <div className="mt-2 ml-6 p-4 bg-steel-50 border border-steel-200 rounded-lg">
      ...
      <p className="mt-2 text-xs text-steel-500">{offer.intro}</p>
      ...
      <div className="mt-4 space-y-3">
        {offer.attachmentPrompts.map((prompt) => {
          ...
          return (
            <div key={prompt.name}>
              <label className="block text-sm font-medium text-ink-700 mb-1">{prompt.label}</label>
              <p className="text-xs text-steel-500 mb-1">{prompt.hint}</p>
              <input
                type="file"
                multiple
                accept="image/jpeg,image/png,application/pdf"
                onChange={(e) => uploadFiles(prompt.name, e.target.files)}
                className="block w-full text-sm text-steel-600 file:mr-3 file:rounded-md file:border-0 file:bg-steel-100 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-ink-800"
              />
              {state?.uploading && <p className="mt-1 text-xs text-steel-500">Uploading…</p>}
              {state?.files.length ? (
                <p className="mt-1 text-xs text-brand">Attached: {state.files.join(", ")}</p>
              ) : null}
              {state?.error && <p className="mt-1 text-xs text-sale-deep">{state.error}</p>}
            </div>
          );
        })}
