  ...
  return (
    <section aria-labelledby="shop-by-category" className="mb-10">
      <div className="mb-4 flex items-end justify-between gap-4">
        <h2 id="shop-by-category" className="text-lg font-semibold text-text-primary">
          {heading}
        </h2>
        <Link
          href="/categories"
          className="inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-accent transition-colors duration-300 hover:text-accent-hover"
        >
          View all
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {categories.map((category) => (
          ...
          >
            ...
            <p className="px-3 py-2.5 text-sm font-semibold leading-snug text-text-primary">
              {category.name}
            </p>
          </Link>
        ))}
