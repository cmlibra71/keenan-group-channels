...
export function QuoteMessages({
  ...
  return (
    <div className="mt-8 border-t border-zinc-200 pt-6">
      <h2 className="mb-3 text-sm font-semibold text-zinc-900">Messages</h2>
      {messages.length > 0 ? (
        <ul className="space-y-3">
          {messages.map((m) => (
            <li key={String(m.id)} className="text-sm">
              ...
              <p className="mt-0.5 whitespace-pre-wrap text-zinc-600">{m.body}</p>
            </li>
          ))}
        </ul>
      ) : (
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="shrink-0 rounded-md bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
            >
