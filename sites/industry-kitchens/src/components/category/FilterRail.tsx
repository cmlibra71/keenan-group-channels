...
function RangeFacet({
  ...
  return (
    <div className="pt-1">
      <div className="mb-2 flex items-center justify-between text-[12px] font-semibold text-[#C73629]">
      ...
      <div className="relative h-6">
        ...
        <div
          ...
          style={{ left: `${left}%`, width: `${width}%`, backgroundColor: "#D94B2B" }}
        />
        {([0, 1] as const).map((which) => (
          <input
            ...
            onKeyUp={release}
            onBlur={release}
            className="pointer-events-none absolute inset-x-0 top-1/2 h-6 w-full -translate-y-1/2 appearance-none bg-transparent [&::-moz-range-thumb]:pointer-events-auto [&::-moz-range-thumb]:h-3.5 [&::-moz-range-thumb]:w-3.5 [&::-moz-range-thumb]:cursor-pointer [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-white [&::-moz-range-thumb]:bg-[#D94B2B] [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:bg-[#D94B2B] [&::-webkit-slider-thumb]:shadow"
          />
        ))}
      </div>
...
function FacetCheckbox({ param, value, label, count }: { param: string; value: string; label: string; count: number }) {
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2 py-0.5 text-[13px] text-zinc-700 hover:text-[#D94B2B]">
      <input
        type="checkbox"
        checked={checked}
    ...
    <div className="flex flex-wrap items-center gap-2">
      {chips.map((chip) => (
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-[#C73629] transition-colors hover:bg-[#D94B2B] hover:text-white"
        >
          {chip.label}
          <X className="h-3 w-3" />
        </button>
