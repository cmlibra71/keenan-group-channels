    ...
    <div className="mt-2">
      <div className="inline-flex items-center gap-1">
        <button
          ...
          onClick={() => step(qty - 1)}
          className="rounded border border-border p-1 text-text-secondary hover:bg-surface-secondary disabled:opacity-40"
          aria-label="Decrease quantity"
        >
          <Minus className="h-3.5 w-3.5" />
        </button>
        <span className="min-w-[3ch] text-center text-sm text-text-primary">{qty}</span>
        <button
          type="button"
          disabled={isPending}
          onClick={() => step(qty + 1)}
        ...
        <button
          ...
          disabled={isPending}
          onClick={remove}
          className="ml-2 inline-flex items-center gap-1 rounded p-1 text-xs text-text-muted hover:bg-red-50 hover:text-red-700 disabled:opacity-40"
