// Generic in-memory sliding-window rate limiter (per container). Same trade-off as
// login-throttle.ts — not shared across replicas, but it blunts abuse of a single
// node (e.g. a customer spamming "Duplicate" to create unbounded quotes).
//
...
export function slidingWindowAllow(
  ...
  // Crude unbounded-growth guard (mirrors login-throttle.ts): once the map is large,
