...
export function QuotePayPanel({
  ...
  return (
    <div className="mt-8 rounded-lg border border-zinc-200 p-5">
      <h2 className="text-base font-semibold text-zinc-900">Pay this quote</h2>

      <div className="mt-3 flex items-baseline justify-between border-b border-zinc-200 pb-3">
        <span className="text-sm text-zinc-600">Amount payable now</span>
        <span className="text-2xl font-semibold text-zinc-900">
          {amountKnown ? amountDue : "To be confirmed"}
        </span>
      </div>
      {depositNote && <p className="mt-2 text-sm text-zinc-600">{depositNote}</p>}
      ...
      {addressSummary ? (
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm text-zinc-700">{addressSummary}</p>
            ...
            {methods.map((m) => (
              <label
                ...
                className={`flex cursor-pointer items-start gap-3 rounded-md border p-3 ${
                  method === m.id ? "border-zinc-900 bg-zinc-50" : "border-zinc-200"
                }`}
              >
                <input
                  type="radio"
                ...
                <span className="min-w-0">
                  <span className="block text-sm font-medium text-zinc-900">{m.name}</span>
                  {m.description && (
      ...
      {selected?.id === "bank_transfer" && selected.bankDetails && (
        <div className="mt-3 rounded-md bg-zinc-50 p-3 text-sm text-zinc-700">
          <p className="font-medium text-zinc-900">Our bank details</p>
          <p className="mt-1">
        ...
        {disabled ? (
          <span className="group relative inline-flex w-full cursor-not-allowed" title={payState.reason}>
            <button
              ...
              aria-disabled="true"
              tabIndex={-1}
              className="pointer-events-none w-full rounded-md bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white opacity-50"
            >
          ...
          <button
            ...
            className="w-full rounded-md bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
          >
            {busy ? "Working…" : amountKnown ? `Pay ${amountDue}` : "Pay this quote"}
          </button>
        )}
        {disabled && <p className="mt-2 text-xs text-zinc-500">{payState.reason}</p>}
      </div>
