...
export default async function QuoteDetailPage({
  ...
  return (
    <AccountShell>
      ...
      {quote.quote_name && (
        <p className="text-base font-medium text-text-primary mb-1">{quote.quote_name}</p>
      )}
            ...
            <div key={item.id} className="p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {chosenOptionLines(quoteLinePicks(item.attributes)).map((label) => (
                  <p key={label} className="text-xs text-text-secondary mt-0.5">
                    + {label}
                  </p>
                ))}
      ...
      {addressSummary && (
        <div className="mt-6">
          <h2 className="text-sm font-semibold text-text-primary mb-1">Delivery address</h2>
          <p className="text-sm text-text-secondary">{addressSummary}</p>
      ...
      {isPricingPendingPayState(payState) ? (
        <p className="mt-8 rounded-lg border border-border p-5 text-sm text-text-secondary">
          {PAY_REASON_PRICING_PENDING}
        </p>
      ) : (
        <QuotePayPanel
