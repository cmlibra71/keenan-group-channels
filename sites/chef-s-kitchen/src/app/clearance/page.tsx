...
export default async function ClearancePage({
  ...
  const heading = specialsView ? "Partner Specials" : activeFilter?.name ?? "Last Units";
          ...
          {specialsLead.products.length > 0 && (
            <section className="mb-10" aria-label="Partner Specials">
              <div className="mb-4 flex items-baseline justify-between gap-3">
                <h2 className="text-lg font-semibold text-ink-900">Partner Specials</h2>
                {specialsLead.total > specialsLead.products.length && (
                  <Link href={`/clearance?type=${SPECIALS_SLUG}`} className="text-sm font-medium text-accent hover:underline">
                    See all {specialsLead.total}
                  </Link>
                )}
              </div>
              <ProductGrid products={specialsLead.products} memberPricingAvailable={memberPricingEnabled} {...(await getListingPricing(specialsLead.products))} listId="partner_specials" listName="Partner Specials" />
            </section>
          )}
          {products.length === 0 ? (
            ...
            <>
              <ProductGrid products={products} memberPricingAvailable={memberPricingEnabled} {...(await getListingPricing(products))} clearance listId="clearance" listName="Clearance" />
