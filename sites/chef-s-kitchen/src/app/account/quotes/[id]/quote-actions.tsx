    ...
    <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-border pt-6">
      {acceptState.kind === "enabled" && (
        <button
          ...
          className="rounded-md bg-text-primary px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
        >
      ...
      {acceptState.kind === "disabled" && (
        ...
        >
          <button
            ...
            aria-disabled="true"
            tabIndex={-1}
            className="pointer-events-none rounded-md bg-text-primary px-4 py-2 text-sm font-medium text-white opacity-50"
          >
            Accept quote
          </button>
          <span
      ...
      {msg && (
        <span className={`text-sm ${msg.kind === "ok" ? "text-text-secondary" : "text-red-600"}`}>{msg.text}</span>
      )}
    </div>
  );
}
...
function RequestButton({
  ...
  if (sent) return <span className="text-sm text-text-muted">{done}</span>;
  ...
  if (!open) {
    return (
      <button
        ...
        className="rounded-md border border-border px-4 py-2 text-sm font-medium text-text-primary hover:bg-surface-secondary"
      >
        {label}
      </button>
    );
