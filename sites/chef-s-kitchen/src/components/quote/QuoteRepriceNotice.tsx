 ...
 *
 * Chef's Depot renders this in the design system's warning tokens rather than
 * the raw Tailwind palette; the wording and the arithmetic are identical.
 *
...
export function QuoteRepriceNotice({
  ...
  return (
    <section className="mb-6 rounded-lg border border-warning/30 bg-warning-bg p-4">
      <h2 className="text-sm font-semibold text-text-primary">Before you accept</h2>
      <p className="mt-1 text-sm text-text-primary">{notice}</p>
          ...
          return (
            <li key={String(line.lineId)} className="flex items-start justify-between gap-4 py-2">
              <div className="min-w-0">
                <p className="truncate text-sm text-text-primary">{line.name ?? line.sku ?? "Item"}</p>
                {line.sku && line.name && (
                  <p className="truncate text-xs text-text-secondary">{line.sku}</p>
                )}
