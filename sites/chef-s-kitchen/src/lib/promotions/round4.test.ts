...
test("checkout judges offers for the same shopper as the cart — account, group and email", () => {
  const src = read("../actions/checkout.ts");
  const call = src.slice(src.indexOf("cartOffers = await resolveCartOffers("), src.indexOf("cartOffers = await resolveCartOffers(") + 400);
  assert.match(call, /accountId: shopper\.accountId/);
  assert.match(call, /email: buyerEmail/);
  assert.match(src, /const shopper = await currentShopperForOffers\(\);/);
  // A guest past a per-customer cap is told and the code removed, never charged a hidden total.
  assert.match(src, /You've already used the code/);
});

test("a refused order is discarded lines-first, not left cancelled in the shopper's list", () => {
  const src = read("../actions/checkout.ts");
  assert.match(src, /await discardUnchargedOrder\(orderId\);/);
});
