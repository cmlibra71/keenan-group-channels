...
function Tile({ category }: { category: BrandCategoryTile }) {
  return (
    <Link
      href={category.href}
      className="group flex items-center gap-3 rounded-lg border border-zinc-200 p-3 transition-all hover:border-zinc-400 hover:shadow-sm"
    >
      {category.image_url && isAllowedImageUrl(category.image_url) ? (
        ...
        <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded bg-zinc-100">
    ...
    <div className="mb-10">
      ...
      {rest.length > 0 && (
        <details className="mt-3 group/all">
          <summary className="inline-flex cursor-pointer list-none items-center gap-1.5 text-sm font-semibold text-zinc-600 hover:text-zinc-900">
