...
export default async function QuoteDetailPage({
  ...
  // render or serialise it, rather than merely left un-rendered (Product Brief §3;
  // same shape of leak as card BIig1Zo1).
  ...
  // `view_company_quotes` can look but not change, the same rule Accept uses.
  const itemsEditable =
  ...
  return (
    <AccountShell>
      ...
      {quote.customer_notes && (
        <div className="mt-6">
          {/* ONE customer-facing word for this field on every screen it appears —
              the drawer's label, this page, the emailed /q/<uuid> copy and the
              printed quote all say "Quote comments" (card 9tbz3sBF). Staff see the
              same text under its staff name, "Customer Note". */}
